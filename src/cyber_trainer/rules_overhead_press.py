from __future__ import annotations

from typing import Dict, List, Optional

from .features import FrontMetrics, SideMetrics, ema
from .rules_common import Hint


class OverheadPressRuleEngine:
    """Simple overhead-press rules:
    - Avoid excessive back arch (side torso angle too small)
    - Keep feet/stance stable (front asymmetry as proxy)
    """

    def __init__(self, frames_required: int = 6) -> None:
        self.frames_required = frames_required
        self._torso_angle_s: Optional[float] = None
        self._asym_s: Optional[float] = None
        self._cnt: Dict[str, int] = {}

    def _count(self, key: str, cond: bool) -> bool:
        if cond:
            self._cnt[key] = self._cnt.get(key, 0) + 1
        else:
            self._cnt[key] = 0
        return self._cnt[key] >= self.frames_required

    def update(self, side: SideMetrics, front: FrontMetrics) -> List[Hint]:
        hints: List[Hint] = []
        if not side.ok:
            return hints

        self._torso_angle_s = ema(self._torso_angle_s, side.torso_hip_angle, alpha=0.35)
        if front.ok:
            self._asym_s = ema(self._asym_s, front.asymmetry_norm, alpha=0.35)

        conf_side = min(1.0, max(0.0, (side.avg_vis - 0.45) / 0.4))
        conf_front = min(1.0, max(0.0, (front.avg_vis - 0.45) / 0.4)) if front.ok else 0.0

        # Back arch (torso angle small indicates rounding/forward bend or arch depending on view)
        if self._torso_angle_s is not None:
            if self._count("ohp_arch", self._torso_angle_s < 100.0):
                hints.append(
                    Hint(
                        ts_ms=side.ts_ms,
                        issue_code="OHP_ARCH",
                        message="Avoid over-arching your lower back when pressing overhead.",
                        severity="warn",
                        view="side",
                        confidence=conf_side,
                    )
                )
                self._cnt["ohp_arch"] = 0

        # Stance asymmetry
        if front.ok and self._asym_s is not None:
            if self._count("ohp_asym", self._asym_s > 0.18):
                hints.append(
                    Hint(
                        ts_ms=front.ts_ms,
                        issue_code="OHP_BALANCE",
                        message="Try to keep weight balanced across both feet.",
                        severity="info",
                        view="front",
                        confidence=conf_front,
                    )
                )
                self._cnt["ohp_asym"] = 0

        return hints
